...
void    mlx_put_image_to_window(mlx_t *mlx, mlx_win_list_t *win_ptr, mlx_img_list_t *img_ptr, int x, int y)
{
  mlx_img_ctx_t	*imgctx;
...
unsigned int    mlx_get_color_value(mlx_t *mlx, int color)
{
  return (color);
}

int mlx_string_put(mlx_t *mlx, mlx_win_list_t *win_ptr, int x, int y, int color, unsigned char *string)
{
  mlx_img_ctx_t	*imgctx;
  int		gX;
  int		gY;
  ...
  imgctx = add_img_to_ctx(mlx->font, win_ptr);
  ...
  while (*string)
      ...
      if (*string >= 32 && *string <= 127)
	{
	  ...
	  [(id)(win_ptr->winid) mlx_gl_draw_font:mlx->font andCtx:imgctx andX:x andY:y andColor:color glyphX:gX glyphY:gY];
#ifdef STRINGPUTX11
	  x += FONT_WIDTH/1.4;
#else
	  x += FONT_WIDTH;
...
int     mlx_destroy_image(mlx_t *mlx, mlx_img_list_t *img_todel)
{
